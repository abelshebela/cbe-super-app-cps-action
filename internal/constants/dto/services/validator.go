...
func (c *CapRequest) Validate() error {
	...
	if err := validation.ValidateStruct(
		...
		validation.Field(&c.SingleCap,
			...
			validation.Min(float64(0)).Error("cap.single_cap must be greater than 0"),
		),
...
func (t *TierRequest) Validate(prev *TierRequest) error {
	...
	if err := validation.ValidateStruct(
		...
		validation.Field(&t.Min,
			validation.Min(float64(0)).Error("tiers.min must be >= 0"),
		),
		validation.Field(&t.Max,
			validation.Required.Error("tiers.max is required"),
			validation.Min(float64(0)).Error("tiers.max must be >= 0"),
		),
...
func (r *CreateServiceRequest) Validate() error {
	if err := validation.ValidateStruct(
		...
		validation.Field(&r.ProductAccount,
			validation.Required.Error("cbe_gl_product_account is required"),
		),
		validation.Field(&r.AbovePaymentType,
			validation.Required.Error("Above payment type is required"),
		),
