...
import (
	...
	"cbe-super-app-cps-action/internal/storage/persistance/cps_roles/core"
...
func (m *cpsRoleStorage) Update(ctx context.Context, id string, req imodel.CPSRoles) error {
	...
	role, err := m.FindByNameOrRoleCode(ctx, req.Name, "")
	...
	if req.Name != "" && role != nil && role.Name != req.Name {
		if err := core.UpdateCpsUsers(ctx, m.client, role.Name, req.Name); err != nil {
			m.logger.Errorf("[CPSRolesStorage][Update] failed to update cps users for role name change: %v", err)
