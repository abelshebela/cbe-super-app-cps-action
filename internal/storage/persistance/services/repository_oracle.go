...
WHERE service_id = HEXTORAW(:1)
  AND is_deleted = 0`
		if _, err := tx.ExecContext(ctx, updateCapsQ, serviceID); err != nil {
			log.Errorf("[ServicesRepo][Delete] update service_cap failed: %v", err)
			return local_util.HandleDBError(err)
		}
...
func (s *ServicesStorage) DeleteServiceKey(ctx context.Context, id string) error {
	...
	if err := tx.QueryRowContext(ctx, checkQ, id).Scan(&count); err != nil {
		log.Errorf("[ServicesRepo][DeleteServiceList] check services failed: %v", err)
		return local_util.HandleDBError(err)
	}
	if count > 0 {
	...
	if err != nil {
		log.Errorf("[ServicesRepo][DeleteServiceList] delete failed: %v", err)
		return local_util.HandleDBError(err)
	}
	rows, _ := res.RowsAffected()
	if rows == 0 {
	...
	if err := tx.Commit(); err != nil {
		log.Errorf("[ServicesRepo][DeleteServiceList] commit failed: %v", err)
		return errors.New(localization.ErrorUnexpectedError.Code)
	}
