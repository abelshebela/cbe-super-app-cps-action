...
func (a *AuthTierStorage) Update(ctx context.Context, id string, authTier *model.AuthTier) error {
	...
	if err != nil {
		log.Errorf("[AuthTierStorage][Update] invalid object id: %v", err)
	...
	if err != nil {
		log.Errorf("[AuthTierStorage][Update] failed to update auth tier: %v", err)
...
func (a *AuthTierStorage) FindByID(ctx context.Context, id string) (*model.AuthTier, error) {
	...
	objID, err := bson.ObjectIDFromHex(id)
	if err != nil {
		log.Errorf("[AuthTierStorage][FindByID] invalid object id: %v", err)
	...
	if err != nil {
		log.Errorf("[AuthTierStorage][FindByID] failed to find auth tier: %v", err)
...
func (a *AuthTierStorage) FindAllWithPagination(ctx context.Context, filterParam types.Filter) (*types.PaginatedResponse[[]model.AuthTier], error) {
	...
	if err != nil {
		log.Errorf("[AuthTierStorage][FindAllWithPagination] failed to fetch auth tiers: %v", err)
		return nil, errors.New(localization.ErrorUnexpectedError.Code)
	}

	total, err := a.dal.TotalCount(ctx, filter)
	if err != nil {
		log.Errorf("[AuthTierStorage][FindAllWithPagination] failed to count auth tiers: %v", err)
		return nil, errors.New(localization.ErrorUnexpectedError.Code)
	}
