...
type hqRepository struct {
	hqDal  dal.MongoDal[dto.HQ, dto.HQ]
	logger utils.Logger
}

func NewHQRepository(client *mongo.Client, dbName string, collection string, logger utils.Logger) storage.HQRepository {
	return &hqRepository{
		hqDal:  dal.NewMongoDal[dto.HQ, dto.HQ](client, dbName, collection),
		logger: logger,
	}
}

func (h *hqRepository) FindOne(ctx context.Context, filter bson.M) (*dto.HQ, error) {
	if filter == nil {
		filter = bson.M{}
	}
