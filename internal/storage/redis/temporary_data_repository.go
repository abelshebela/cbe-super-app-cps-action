...
func (t *TemporaryDataRepository) SaveTemporaryData(ctx context.Context, key string, data map[string]interface{}, expiration time.Duration) error {
	...
	for field, value := range data {
		...
		if err != nil {
			log.Errorf("Failed to save temporary data field %s for key %s: %v", field, key, err)
			return err
		}
	}
	...
	if err != nil {
		log.Errorf("Failed to set expiration for temporary data %s: %v", key, err)
		return err
	}

	log.Infof("Successfully saved temporary data: %s", key)
	return nil
}
...
func (t *TemporaryDataRepository) GetTemporaryData(ctx context.Context, key string) (map[string]string, error) {
	...
	if err != nil {
		log.Errorf("Failed to get temporary data %s: %v", key, err)
		return nil, err
	}

	log.Infof("Successfully retrieved temporary data: %s", key)
	return data, nil
}
...
func (t *TemporaryDataRepository) DeleteTemporaryData(ctx context.Context, key string) error {
	...
	if err != nil {
		log.Errorf("Failed to delete temporary data %s: %v", key, err)
		return err
	}

	log.Infof("Successfully deleted temporary data: %s", key)
	return nil
}
...
func (t *TemporaryDataRepository) UpdateTemporaryData(ctx context.Context, key string, updates map[string]interface{}) error {
	...
	for field, value := range updates {
		err := t.redisRepo.HSet(ctx, redisKey, field, value)
		if err != nil {
			log.Errorf("Failed to update temporary data field %s for key %s: %v", field, key, err)
			return err
		}
	}

	log.Infof("Successfully updated temporary data: %s", key)
	return nil
}
...
func (t *TemporaryDataRepository) SaveFormData(ctx context.Context, sessionID string, formData map[string]interface{}, expiration time.Duration) error {
	...
	for field, value := range formData {
		...
		if err != nil {
			log.Errorf("Failed to save form data field %s for session %s: %v", field, sessionID, err)
			return err
		}
	}
	...
	if err != nil {
		log.Errorf("Failed to set expiration for form data %s: %v", sessionID, err)
		return err
	}

	log.Infof("Successfully saved form data for session: %s", sessionID)
	return nil
}
...
func (t *TemporaryDataRepository) GetFormData(ctx context.Context, sessionID string) (map[string]string, error) {
	...
	if err != nil {
		log.Errorf("Failed to get form data for session %s: %v", sessionID, err)
		return nil, err
	}

	log.Infof("Successfully retrieved form data for session: %s", sessionID)
	return formData, nil
}
...
func (t *TemporaryDataRepository) DeleteFormData(ctx context.Context, sessionID string) error {
	...
	if err != nil {
		log.Errorf("Failed to delete form data for session %s: %v", sessionID, err)
		return err
	}

	log.Infof("Successfully deleted form data for session: %s", sessionID)
	return nil
}
...
func (t *TemporaryDataRepository) SetCache(ctx context.Context, key string, value interface{}, expiration time.Duration) error {
	...
	if err != nil {
		log.Errorf("Failed to set cache for key %s: %v", key, err)
		return err
	}

	log.Infof("Successfully set cache: %s", key)
	return nil
}
...
func (t *TemporaryDataRepository) GetCache(ctx context.Context, key string) (string, error) {
	...
	if err != nil {
		log.Errorf("Failed to get cache for key %s: %v", key, err)
		return "", err
	}

	log.Infof("Successfully retrieved cache: %s", key)
	return value, nil
}
...
func (t *TemporaryDataRepository) DeleteCache(ctx context.Context, key string) error {
	...
	if err != nil {
		log.Errorf("Failed to delete cache for key %s: %v", key, err)
		return err
	}

	log.Infof("Successfully deleted cache: %s", key)
	return nil
}
...
func (t *TemporaryDataRepository) ClearCacheByPattern(ctx context.Context, pattern string) error {
	...
	if err != nil {
		log.Errorf("Failed to get cache keys for pattern %s: %v", pattern, err)
		return err
	}

	if len(keys) == 0 {
		log.Infof("No cache keys found for pattern: %s", pattern)
		return nil
	}
	...
	for _, key := range keys {
		err := t.redisRepo.Delete(ctx, key)
		if err != nil {
			log.Errorf("Failed to delete cache key %s: %v", key, err)
			return err
		}
	}

	log.Infof("Successfully cleared %d cache keys for pattern: %s", len(keys), pattern)
	return nil
}
