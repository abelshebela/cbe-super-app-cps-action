...
func (o *OTPSessionRepository) SaveOTP(ctx context.Context, phoneNumber string, otp string, expiration time.Duration) error {
	...
	if err != nil {
		log.Errorf("Failed to save OTP for phone number %s: %v", phoneNumber, err)
		return err
	}

	log.Infof("Successfully saved OTP for phone number: %s", phoneNumber)
	return nil
}
...
func (o *OTPSessionRepository) GetOTP(ctx context.Context, phoneNumber string) (string, error) {
	...
	if err != nil {
		log.Errorf("Failed to get OTP for phone number %s: %v", phoneNumber, err)
		return "", err
	}

	log.Infof("Successfully retrieved OTP for phone number: %s", phoneNumber)
	return otp, nil
}
...
func (o *OTPSessionRepository) DeleteOTP(ctx context.Context, phoneNumber string) error {
	...
	if err != nil {
		log.Errorf("Failed to delete OTP for phone number %s: %v", phoneNumber, err)
		return err
	}

	log.Infof("Successfully deleted OTP for phone number: %s", phoneNumber)
	return nil
}
...
func (o *OTPSessionRepository) IncrementOTPAttempts(ctx context.Context, phoneNumber string) (int, error) {
	...
	if err != nil {
		...
		if err != nil {
			log.Errorf("Failed to set initial OTP attempts for phone number %s: %v", phoneNumber, err)
			return 0, err
		}
		log.Infof("Set initial OTP attempts for phone number: %s", phoneNumber)
		return 1, nil
	}

	currentAttempts, err := strconv.Atoi(currentAttemptsStr)
	if err != nil {
		log.Errorf("Failed to parse OTP attempts for phone number %s: %v", phoneNumber, err)
		return 0, err
	}
	...
	if err != nil {
		log.Errorf("Failed to increment OTP attempts for phone number %s: %v", phoneNumber, err)
		return 0, err
	}

	log.Infof("Incremented OTP attempts for phone number %s: %d", phoneNumber, newAttempts)
...
func (o *OTPSessionRepository) GetOTPAttempts(ctx context.Context, phoneNumber string) (int, error) {
	...
	if err != nil {
		log.Warnf("No OTP attempts found for phone number %s", phoneNumber)
		return 0, nil
	}
	...
	log.Infof("Retrieved OTP attempts for phone number %s: %d", phoneNumber, attempts)
	return attempts, nil
}
...
func (o *OTPSessionRepository) ResetOTPAttempts(ctx context.Context, phoneNumber string) error {
	...
	err := o.redisRepo.Delete(ctx, key)
	if err != nil {
		log.Errorf("Failed to reset OTP attempts for phone number %s: %v", phoneNumber, err)
		return err
	}

	log.Infof("Successfully reset OTP attempts for phone number: %s", phoneNumber)
	return nil
}
...
func (o *OTPSessionRepository) SaveOTPVerification(ctx context.Context, phoneNumber string, verified bool, expiration time.Duration) error {
	...
	if err != nil {
		log.Errorf("Failed to save OTP verification for phone number %s: %v", phoneNumber, err)
		return err
	}

	log.Infof("Successfully saved OTP verification for phone number: %s", phoneNumber)
	return nil
}
...
func (o *OTPSessionRepository) GetOTPVerification(ctx context.Context, phoneNumber string) (bool, error) {
	...
	if err != nil {
		log.Warnf("No OTP verification found for phone number %s", phoneNumber)
		return false, nil
	}

	verified, err := strconv.ParseBool(verifiedStr)
	if err != nil {
		log.Errorf("Failed to parse OTP verification for phone number %s: %v", phoneNumber, err)
		return false, err
	}

	log.Infof("Retrieved OTP verification for phone number %s: %t", phoneNumber, verified)
	return verified, nil
}
...
func (o *OTPSessionRepository) DeleteOTPVerification(ctx context.Context, phoneNumber string) error {
	...
	err := o.redisRepo.Delete(ctx, key)
	if err != nil {
		log.Errorf("Failed to delete OTP verification for phone number %s: %v", phoneNumber, err)
		return err
	}

	log.Infof("Successfully deleted OTP verification for phone number: %s", phoneNumber)
	return nil
}
