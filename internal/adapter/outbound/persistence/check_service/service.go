...
func (sp *servicePersistence) UpdateSingleMaxTransfer(ctx context.Context, id string, req any) error {
	...
	data, err := local_utils.JsonUnmarshal[model.Service](req)
	if err != nil {
		sp.logger.Errorf("error unmarshalling request for update single max transfer: %v", err)
		return err
	}
	_, err = sp.ValidateMaxTotalCap(ctx, data.Cap)
	if err != nil {
		sp.logger.Errorf("validation failed for max total cap in update single max transfer: %v", err)
		return err
	}

	err = sp.createCpsAction(ctx, id, prev, req, string(model.RequestUpdateServiceSingle))
	if err != nil {
...
func (sp *servicePersistence) UpdateMinimumTransferCap(ctx context.Context, id string, req any) error {
	...
	err = sp.createCpsAction(ctx, id, prev, reqData, string(model.RequestUpdateServiceMinCap))
	if err != nil {
...
func (sp *servicePersistence) Authorize(ctx context.Context, cpsAction any) (any, error) {
	fmt.Println("***********************")
	action, ok := cpsAction.(*model.CPSAction)
	if !ok || action == nil {
	...
	case string(model.RequestDeleteServiceFee):
		...
		// If there is a field to pull from an array, handle it here
		// For example, if current contains "remove_from_array": {"field": "fees", "value": "123"}
		if remove, ok := current["remove_from_array"].(map[string]interface{}); ok {
