...
import (
	...
	"gitlab.com/bersufekadgetachew/cbe-super-app-cps-action/pkg/entities"
...
type ServiceRepo struct {
	client      *mongo.Client
	logger      utils.Logger
	cpsDal      dal.MongoDal[entities.Service, entities.Service]
	customerDal dal.MongoDal[member.User, member.User]
}

var _ outbound.FaydaAccountRepository = (*ServiceRepo)(nil)

func InitServicePersistence(client *mongo.Client, database string, cpsCollection []string, logger utils.Logger) *ServiceRepo {
...
func (f *ServiceRepo) InitiateServiceFee(ctx context.Context, req entity.CPSAction) (*entities.Service, error) {
	filter := bson.M{
...
func (f *ServiceRepo) AuthorizeServiceFee(ctx context.Context, req entity.CPSAction) (*entity.CPSAction, error) {
	filter := bson.M{
...
func (f *ServiceRepo) RejectServiceFee(ctx context.Context, req entities.Service) (*entities.Service, error) {
	filter := bson.M{
		"action_data.phone_number": req.ActionData.PhoneNumber,
		"department":               req.Department,
		"status":                   entity.ActionPending,
