...
func (s *branchService) FilterSingleBranches(ctx context.Context, region, district string) ([]string, error) {
    if region == "" || district == "" {
...
func (s *branchService) DisableSingleBranch(ctx context.Context, branchCode, cpsData string) (*entities.CPSAction, error) {
    if branchCode == "" || cpsData == "" {
        return nil, fmt.Errorf("branchCode and bpsData are required")
    }
...
func (s *branchService) DisableMultipleBranches(ctx context.Context, branchCodes []string, cpsData string) (*entities.CPSAction, error) {
    if len(branchCodes) == 0 || cpsData == "" {
        return nil, fmt.Errorf("branchCodes and cpsData are required")
    }
    return s.repo.DisableMultipleBranches(ctx, branchCodes, cpsData)
}
