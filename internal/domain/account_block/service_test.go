...
import (
	...
	"cbe-super-app-cps-action/internal/domain/account_block/mocks"
...
func TestAccountService_ApproveRegionBlock(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:     "success",
			actionID: "action3",
			approve:  false, // testing rejection path
			reason:   &reason,
			mockSetup: func() {
				mockRepo.EXPECT().
					ApproveRegionBlock(ctx, "action3", false, &reason, checker).
					Return(nil)
			},
			wantErr: false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tt.mockSetup()
			err := service.ApproveRegionBlock(ctx, tt.actionID, tt.approve, tt.reason, checker)
			if tt.wantErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
