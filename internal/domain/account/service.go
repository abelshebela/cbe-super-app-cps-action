...
type AccountService struct {
    ...
    apiClient  accountPort.AccountAPIClient
    logger     utils.Logger
}

func NewAccountService(repository Repository, apiClient accountPort.AccountAPIClient, logger utils.Logger) *AccountService {
...
func (s *AccountService) CreateAccount(ctx context.Context, userID string) (*AccountCreationResult, error) {
    ...
    if err != nil {
        ...
        if errors.Is(err, local.DefineError.Account["PHONE_LOOKUP_FAILED"]) {
            return nil, NewlocalServiceError(local.DefineError.Account["PHONE_LOOKUP_FAILED"])
        }
        return nil, NewlocalServiceError(local.DefineError.Account["API_REQUEST_FAILED"])
    }
    if exists {
        s.logger.Warnf("Phone number %s already exists for user.PhoneNumber", userID)
    ...
    s.logger.Errorf("Failed to update user %s", userID, err.Error())
...
if user.RegistrationType != "" {
    ...
    if err := s.repository.CreateLinkedAccount(ctx, linkedAccount); err != nil {
        s.logger.Errorf("Failed to create linked account for user %s: %s", err.Error())
        return nil, NewServiceError(common.DefineError.General["GENERAL_DB_INSERT"])
    }
