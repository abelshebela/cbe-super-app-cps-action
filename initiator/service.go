...
func InitServiceLayer(mongoClient *mongo.Client, persistence persistance.Persistence, oracle OraclePersistence, logger utils.Logger, sitotagRPCClient transactionpb.TransactionServiceClient, cfg *config.VaultConfig, minioClient *s3.Client, redis storage.RedisRepository, smsService *lib.NotificationStore, clientOrchestrationProducer *kafka.ClientOrchestrationProducer, presignClient *s3.PresignClient, queueManager *queue.QueueManager) service.ServiceLayer {
	...
	bank_service := bankService.NewBankService(logger, persistence.BankPersistence, persistence.BankOraclePersistence, nil, minioClient, minioPubUrl, cfg, cfg.S3BucketName)
	...
	cpsActionService := cpsaction.NewCPSActionService(persistence.CPSActionRolePersistence, persistence.CPSAction, logger, *dispatcher, minioClient, cfg.S3BucketName, cfg.MinioPublicEndPoint, presignClient)
	...
	bank_service = bankService.NewBankService(logger, persistence.BankPersistence, persistence.BankOraclePersistence, cpsActionService, minioClient, minioPubUrl, cfg, cfg.S3BucketName)
