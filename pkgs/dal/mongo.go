...
// Private helper for find logic
func (m *mongoDal[T, K]) findAllHelper(ctx context.Context, filter, projection bson.M, opts *options.FindOptionsBuilder) ([]*K, error) {
...
func (m *mongoDal[T, K]) FindAll(ctx context.Context, filter, projection bson.M) ([]*K, error) {
	...
	return m.findAllHelper(ctx, filter, projection, opts)
}
...
func (m *mongoDal[T, K]) DeleteOneHard(ctx context.Context, filter bson.M) error {
	_, err := m.collection.DeleteOne(ctx, filter)
	return err
}

// Private helper for delete logic
func (m *mongoDal[T, K]) deleteHelper(ctx context.Context, filter bson.M, hard bool) error {
	if hard {
		_, err := m.collection.DeleteOne(ctx, filter)
		return err
	}
...
func (m *mongoDal[T, K]) DeleteOne(ctx context.Context, filter bson.M) error {
	return m.deleteHelper(ctx, filter, false)
}

// func (m *mongoDal[T, K]) DeleteOneHard(ctx context.Context, filter bson.M) error {
// 	return m.deleteHelper(ctx, filter, true)
// }
